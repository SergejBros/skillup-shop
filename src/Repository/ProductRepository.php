    ...
    public function findByAttributes(Category $category, array $filter)
    {
       ...
       $usedAttributesCount = 0;
        ...
        foreach ($category->getAttributes() as $attribute) {
            ...
            $attributeExpression = $attributeQueryBuilder->expr()->andX();
            $attributeExpression->add('v.attribute = ' . $attribute->getId());

            if($minValue){
                $attributeExpression->add('v.value >= ' . floatval($minValue));
            }

            if($maxValue){
                $attributeExpression->add('v.value <= ' . floatval($maxValue));
            }

            $attributeQueryBuilder->orWhere($attributeExpression);
       }
